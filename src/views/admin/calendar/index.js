...
import { splitParamsUrl } from "@utils/common.js";
...
class CalendarSchedule extends React.Component {
  constructor(prop) {
    ...
    if (window.location.search) {
      const paramStr = splitParamsUrl(window.location.search);
  ...
  componentDidMount() {
    this.createCalendar();
    scheduleProvider
      .search({ page: 0, size: 999999, courseId: this.courseId })
      .then((json) => {
        if (json && json.code === 200) {
          ...
          json.data.map((item) => {
            JSON.parse(item.dates).map((date) => {
              calendar.addEvent({
                title: item.subjectInfo && item.subjectInfo.name,
                start: date,
                end: date,
                ...
                <FormGroup className="mb-0">
                  ...
                  >
                    ...
                    <Button
                      ...
                      color=""
                      type="button"
                      onClick={() => this.setState({ radios: "bg-default" })}
                    />
                    <Button
